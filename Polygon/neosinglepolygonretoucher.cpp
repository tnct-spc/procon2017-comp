...
#include "ui_neosinglepolygonretoucher.h"

NeoSinglePolygonRetoucher::NeoSinglePolygonRetoucher(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NeoSinglePolygonRetoucher)
{
    ui->setupUi(this);
}

NeoSinglePolygonRetoucher::~NeoSinglePolygonRetoucher()
{
    delete ui;
}

std::unique_ptr<NeoSinglePolygonRetoucher> NeoSinglePolygonRetoucher::createInstance(procon::ExpandedPolygon const& polygon_, cv::Mat const& image_, std::string wname_, int scale_)
{
    std::unique_ptr<NeoSinglePolygonRetoucher> instance(new NeoSinglePolygonRetoucher());
    instance->polygon = polygon_;
    instance->image = QOpenCVImage::CVMatToQImage(image_);
    instance->wname = wname_;
    instance->scale = scale_;
    instance->setWindowTitle(QString::fromStdString(wname_));
    return std::move(instance);
}

void NeoSinglePolygonRetoucher::paintEvent(QPaintEvent *)
{
